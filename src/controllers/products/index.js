...
app.get("/", App.Core.AuthRoute, async (req, res) => {
	try {
		...
		const products = await App.Services.Product.list(
			App.Core.cleanObject(req.query)
		)
...
app.post(
	...
	App.Core.AuthRoute,
	App.Core.AdminRoute,
	App.Core.FormDataRoute,
	async (req, res) => {
		try {
			const Product = await App.Services.User.addProduct(
				req.fields,
				req.files.images
			)
...
app.get("/:id", App.Core.FormDataRoute, async (req, res) => {
	try {
...
app.put("/:id", App.Core.AuthRoute, App.Core.AdminRoute, async (req, res) => {
	try {
		const Product = await App.Services.User.editProduct(
			req.params.id,
			req.body
		)
		res.status(200).send(Product)
